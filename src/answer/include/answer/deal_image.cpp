...
void
DealImage::Invoking(cv::Mat image_copy, std::vector<cv::Vec4i> &lines, std::vector<std::vector<cv::Point>> &contours) {
    DealImage::SetImage(image_copy, lines, contours);
}

void
DealImage::SetImage(cv::Mat image_copy, std::vector<cv::Vec4i> &lines, std::vector<std::vector<cv::Point>> &contours) {
    //change to HSV
